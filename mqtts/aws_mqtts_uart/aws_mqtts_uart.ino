...
void setup() {
  ...
  enterCommandMode();               // Send +++ command
  at_set("OP", "6");                // MQTTS mode
  at_set("LI", "192.168.11.2");     // Local IP
  at_set("SM", "255.255.255.0");    // Subnet mask
  at_set("GW", "192.168.11.1");     // Gateway
  at_set("DS", "8.8.8.8");          // DNS
  at_set("QO", "0");                // MQTT QoS level
  at_set("RC", "2");                // Root CA verify required
  at_set("CE", "1");                // Client certificate enable
  at_set("OC", ROOTCA);             // Root CA
  at_set("LC", CLIENT_CERT);        // Client cert
  at_set("PK", PRIVATE_KEY);        // Private key
  at_set("RH", AWS_IOT_ENDPOINT);   // AWS endpoint
  at_set("RP", AWS_MQTT_PORT);      // AWS port (8883)
  ...
  Serial.println("=== UART MQTTs Client (AWS IoT Core) ===");
