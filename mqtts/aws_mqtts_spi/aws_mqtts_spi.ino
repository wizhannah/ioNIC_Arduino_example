...
void setup() {
  ...
  at_set("OP6");            // MQTTS mode
  at_set("LI192.168.11.2"); // Local IP
  at_set("SM255.255.255.0");
  at_set("GW192.168.11.1");
  at_set("DS8.8.8.8");
  at_set("QO0");                      // Set MQTT QoS level
  at_set("RC2");                      // Set Root CA verify option : required
  at_set("CE1");                      // Set Client certificate enable
  at_set((String("OC") + ROOTCA).c_str());
  at_set((String("LC") + CLIENT_CERT).c_str());
  at_set((String("PK") + PRIVATE_KEY).c_str());
  at_set((String("RH") + AWS_IOT_ENDPOINT).c_str());
  at_set((String("RP") + AWS_MQTT_PORT).c_str());
  ...
  Serial.println("=== SPI MQTTs Client (AWS IoT Core) ===");
  delay(3000);
}
