...
static void sendHttpResponse() {
  ...
  body += "</head><body>";
  body += "<h1>ioNIC Web Server (SPI)</h1>";
  body += "</body></html>";
  ...
  data_tx((const uint8_t *)header.c_str(), (uint16_t)header.length());
  data_tx((const uint8_t *)body.c_str(), (uint16_t)body.length());
}
...
void setup() {
  ...
  Serial.println("=== SPI HTTP Web Server ===");
