...
static const char *REMOTE_HOST = "192.168.11.162";
static const char *REMOTE_PORT = "443";
static const char *REPLY_ON_RX = "ioNIC test\r\n";
...
void setup() {
  ...
  Serial.println("If your SSL server requires certificates,");
  Serial.println("set ROOT_CA_PEM / CLIENT_CERT_PEM / PRIVATE_KEY_PEM");
  Serial.println("and the sketch will send OC/LC/PK commands.");

  // Auto configuration on boot
  enterCommandMode();              // Send +++ command
  factory_reset();                 // Send Factory Reset command
  enterCommandMode();              // Send +++ command
  at_set("OP", "4");               // SSL TCP client mode
  ...
  at_set("RH", REMOTE_HOST);       // Remote host (SSL server)
  at_set("RP", REMOTE_PORT);       // Remote port (SSL server)
  at_set("SV", nullptr);           // Save
  delay(100);
  Serial.println("Auto-configuration done. Module will reboot.");
  device_reset();                  // Reset device

  Serial.println("=== UART TLS Client ===");
...
void loop() {
  ...
  uint8_t c;
  while (rxBufferPop(c)) {
    Serial.write((char)c); // Print TLS response to USB.
    if (c == '\n') {
      uart_puts(UART_ID, REPLY_ON_RX);
    }
  }
}
