...
void setup() {
  ...
  at_set("OP4");               // SSL TCP client mode
  ...
  at_set((String("RH") + REMOTE_HOST).c_str());  // Remote host (SSL server)
  at_set((String("RP") + REMOTE_PORT).c_str());  // Remote port (SSL)
  at_set("SV");                // Save
  Serial.println("Auto-configuration done. Module will reboot.");
  at_set("RT");
  delay(3000);

  Serial.println("=== SPI TLS Client ===");
  Serial.println("Pins: SCK=GPIO2, MISO=GPIO4, MOSI=GPIO3, CS=GPIO5, INT=GPIO26");
  delay(3000);
...
void loop() {
  ...
  if (spiRxPending || digitalRead(PIN_INT) == LOW) {
    ...
    while (digitalRead(PIN_INT) == LOW) {
      ...
      for (int i = 0; i < n; ++i) {
        if (dataBuf[i] == '\n') {
          data_tx((const uint8_t *)REPLY_ON_RX, (uint16_t)strlen(REPLY_ON_RX));
        }
      }
    }
    spiRxPending = false;
