...
static void sendHttpGet() {
  ...
  req += "User-Agent: pico-uart-http/1.0\r\n";
  req += "Connection: close\r\n\r\n";
  uart_puts(UART_ID, req.c_str());
}
...
void setup() {
  ...
  at_set("RH", REMOTE_HOST);       // Remote host (HTTP server)
  at_set("RP", REMOTE_PORT);       // Remote port (HTTP)
  at_set("SV", nullptr);           // Save
  delay(100);
  Serial.println("Auto-configuration done. Module will reboot.");
  device_reset();                  // Reset device

  Serial.println("=== UART HTTP Client ===");
...
void loop() {
  ...
  uint8_t c;
  while (rxBufferPop(c)) {
    Serial.write((char)c); // Print HTTP response to USB.
  }
}
