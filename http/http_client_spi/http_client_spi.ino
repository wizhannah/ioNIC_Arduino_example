...
static void sendHttpGet() {
  String req = String("GET ") + HTTP_PATH + " HTTP/1.1\r\n";
  req += String("Host: ") + REMOTE_HOST + ":" + REMOTE_PORT + "\r\n";
  req += "User-Agent: pico-spi-http/1.0\r\n";
  req += "Connection: close\r\n\r\n";

  data_tx((const uint8_t *)req.c_str(), (uint16_t)req.length());
}
...
void setup() {
  ...
  at_set((String("RH") + REMOTE_HOST).c_str());  // Remote host (HTTP server)
  at_set((String("RP") + REMOTE_PORT).c_str());  // Remote port (HTTP)
  at_set("SV");                // Save
  Serial.println("Auto-configuration done. Module will reboot.");
  at_set("RT");
  delay(3000);

  Serial.println("=== SPI HTTP Client ===");
