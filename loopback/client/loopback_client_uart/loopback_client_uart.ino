...
void setup() {
  ...
  at_set("RH", "192.168.11.162");  // Remote host (server)
  at_set("RP", "5000");            // Remote port
  at_set("SV", nullptr);           // Save
  delay(100);
  Serial.println("Auto-configuration done. Module will reboot.");
  device_reset();                  // Reset device

  Serial.println("=== UART Loopback Client ===");
...
void loop() {
  while (Serial.available() > 0) {
    ...
    uart_putc_raw(UART_ID, c); // Send USB data to UART peer.
  }

  uint8_t c;
  while (rxBufferPop(c)) {
    Serial.write((char)c); // Print UART response to USB.
  }
}
